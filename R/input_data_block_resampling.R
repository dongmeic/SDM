...
split.xy <- function(df, set, year){
	X <- df[,-1]
	y <- df[,1]
	write.csv(X, paste0("split/input/X_",set,"_",year,".csv"), row.names=FALSE)
	write.csv(y, paste0("split/input/y_",set,"_",year,".csv"), row.names=FALSE)
}
...
foreach (i=3:nyr)%dopar%{
  ...
  test.idx <- sample(unique(ndf$xy),round(0.2*n))
  train.idx <- unique(ndf$xy)[!(unique(ndf$xy) %in% test.idx)]
